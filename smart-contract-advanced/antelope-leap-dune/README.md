...
일일히 노드를 설정하는 작업은 번거롭기 때문에 ENF 는 Docker 컨테이너 환경에서 개발자들이 쉽게 Antelope 로컬 노드를 구성하고 스마트 컨트랙트를 테스트 할 수 있도록 DUNE(Docker Utilities for Node Execution) 이라는 도구를 만들고 공개했습니다.
