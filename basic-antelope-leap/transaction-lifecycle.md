...
* `name name` - 다소 혼란스러운 액션 이름이며, `name` 타입 입니다. 일반적으로 WASM 코드에는 액션 이름에 따라 다른 함수를 호출하는 디스패처가 있습니다.
* `vector<permission_level> authorisation` - 이 실행을 인증하는 계정 및 권한을 정의합니다. 트랜잭션에는 인증 요구 사항을 충분히 충족할 수 있는 서명이 포함되어야 합니다.
* `bytes data` - 액션 인수의 바이트 벡터입니다. 일반적으로 이것은 컨트랙트 계정에 의해 선언된 ABI에 해당하는 직렬화된 구조입니다. 하지만 `nodeos` 는 보내는 타이밍에 무슨 데이터가 들어 있는지 신경 쓰지 않습니다. 데이터 내용을 역직렬화하고 해석하는 것은 전적으로 WASM 코드가 하는 일입니다.
...
위에서 설명한 것처럼 여러가지 이유로 트랜잭션이 실패하곤 합니다.&#x20;

RPC 노드에서 클라이언트는 트랜잭션이 실패하면 바로 알 수 있습니다. 일반적으로 다음과 같은 이유로 트랜잭션이 실패합니다.

* 토큰 잔액이 너무 낮거나 올바르지 않은 액션 인수와 같은 로직 문제가 발생할 경우 스마트 컨트랙트는 트랜잭션을 거부할 수 있습니다.(check(false)를 호출).&#x20;
* 트랜잭션을 실행하는 데 30ms 이상이 소요되면 실패합니다. 30ms는 트랜잭션의 최대 실행 시간입니다. 또한 실행 시간은 물리적 서버에 종속적이기 때문에, 만약 RPC 노드에서 트랜잭션 실행에 29ms 가 소요되었다면, 실행이 되더라도 트랜잭션이 블록 생성자 노드로 이동하는 과정에서 추가로 시간이 소요되어 결과적으로 실패할 가능성이 높습니다.&#x20;
* 트랜잭션을 승인할 수 없는 부적절한 서명 때문에 실패할 수 있습니다. 예를 들어, 토큰 전송을 결제 계정으로만 승인하도록 설정한 경우가 있습니다.&#x20;
* RPC 노드가 주관적 청구(Subjective Billing)를 사용하도록 설정되어 있을 수 있습니다. 주관적 청구는 이전에 실패한 트랜잭션을 참고한 다음, 보낸 사람에게 마치 해당 트랜잭션이  성공한 것처럼 남은 CPU 리소스를 계산합니다. 따라서, 어떤 사용자가 다량의 실패한 트랜잭션을 발송한다면 곧 주관적 청구에 의해 차단될 것입니다.&#x20;

일단 트랜잭션이 RPC 노드에 전달되면 네트워크의 다른 p2p 노드로 전송됩니다. 각 노드는 다음과 같은 몇 가지 이유로 트랜잭션을 중단할 수도 있습니다.&#x20;

* 트랜잭션이 로컬 노드에서 30ms보다 오래 걸리는 경우. 노드의 CPU 속도가 느리거나, 노드에서 Optimized Compiler를 사용하지 않도록 설정(`eos-vm-oc-enable = false`)했거나, 다른 작업 때문에 시스템 리소스가 충분치 않을 수 있습니다.&#x20;
* 스마트 컨트랙트 로직이 새로운 조건에서 트랜잭션을 수락하지 않을 수 있습니다. 예를 들어 블록에 도착한 다른 누군가의 트랜잭션이 스마트 컨트랙트 테이블을 업데이트 하여, 그 결과로 우리 쪽의 추측성 트랜잭션이 더 이상 실행될 수 없게 되는 경우입니다.&#x20;
* p2p를 통해 도착하는 트랜잭션에 대해 주관적 청구를 사용할 수 있으므로 노드는 이전의 계정 활동(실패한 트랜잭션을 포함)에서 CPU 리소스를 차감합니다.

트랜잭션이 블록 생산자(BP) 노드에 도착했다 하더라도 여전히 트랜잭션은 실패할 수 있습니다.&#x20;

* BP 노드도 p2p 인터페이스를 통해 트랜잭션을 수신하므로 위에서 설명한 모든 이유가 블록 생산자 노드에서도 발생할 수 있습니다.&#x20;
* BP 중 하나가 오프라인 상태가 되어버리면 어떤 블록 사이에 6초간의 간격이 발생할 수 있습니다. 만약 트랜잭션 만료 시간이 너무 짧으면 이 시간 범위 내에서 만료 버릴 수도 있습니다.&#x20;
* 블록체인에서 마이크로포크가 발생하면, 현재 BP는 대기열의 모든 트랜잭션을 다시 처리해야 합니다. 포크의 깊이에 따라 몇 초 또는 수십 초 전에 전송된 트랜잭션에 영향을 미칠 수 있기 때문에, 다시 평가 하는 동안 트랜잭션이 만료될 수도 있습니다.&#x20;
* 마이크로포크 중에 추측성 트랜잭션은 임의의 순서로 다시 처리됩니다. 이 때 스마트 컨트랙트 로직이 잘못된 순서로 평가되면 트랜잭션이 거부될 수 있습니다. \
  예를 들어, 1. Alice가 Bob에게 10개의 토큰을 보낸 다음(Alice 가 토큰을 보내기 전에는 Bob의 계정 잔고가 10개 미만), 2. 다시 Bob 이 Chris에게 10개의 토큰을 보내는 트랜잭션이 있다고 합시다. 포크가 발생하면, 예를 들어 BP가 두 번째 트랜잭션을 먼저 평가하려고 할 수 있습니다. 이 경우 Bob의 잔액이 10보다 낮기 때문에 전송을 실행할 수 없습니다.

## 트랜잭션 라이프사이클 다루기&#x20;

`nodeos` 소프트웨어는 트랜잭션 상태를 추적하고 손실되지 않도록 하는 다양한 방법을 제공합니다. `state_history_plugin` 과 `trace_api_plugin` 두 가지 모두 각 블록에 속한 모든 트랜잭션을 내보내고 마이크로포크에서 롤백합니다. 이들은 블록체인 노드에 직접 액세스해야 할 필요가 있지만 중요한 트랜잭션의 상태를 추적하는 데 사용할 수 있습니다.

Leap 3.1 부터 다음과 같은 새로운 RPC API가 도입되었습니다.&#x20;

* `/v1/chain/send_transaction2`\
  트랜잭션이 블록에 포함될 때까지 재전송을 시도.&#x20;
* `/v1/chain/get_transaction_status`\
  트랜잭션 ID로 질의 하여 트랜잭션 ID의 현재 상태를 받을 수 있음.\
  (예: `UNKNOWN`, `FAILED`, `FORKED_OUT`, `LOCALLY_APPLIED`, `IN_BLOCK`, `IRREVERSIBLEUNKNOWN`, `IRREVERSIBLE`.)
