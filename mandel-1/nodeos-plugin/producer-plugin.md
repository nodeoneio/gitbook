...
`producer_plugin` 에 지연된 트랜잭션이 대기열에 있을 경우, 트랜잭션 유형 중 하나에 다른 트랜잭션 유형보다 높은 우선 순위를 줄 수 있습니다. 아래 옵션은 지연된 트랜잭션과 수신되는 트랜잭션 간의 비율을 설정합니다.
...
'arg'가 충분히 큰 숫자로 설정된 경우 플러그인은 트랜잭션의 대기열이 빌 때까지 항상 수신되는 트랜잭션을 먼저 처리합니다. 'arg'가 0이면 `producer_plugin` 은 대기열의 지연된 트랜잭션을 먼저 처리합니다.
